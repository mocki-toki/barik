...
struct TimeWidget: View {
    ...
    var dateFormat: String { config["date-format"]?.stringValue ?? "E d MMM" } // Default format for date
    var timeFormat: String { config["time-format"]?.stringValue ?? "J:mm" } // Default format for time
    ...
    var body: some View {
        VStack(alignment: .trailing, spacing: 2) {
            HStack {
                Text(formattedDate(from: currentTime))
    ...
    private func formattedDate(from time: Date) -> String {
        return formatTime(pattern: dateFormat, from: time)
    }

    // Format the time separately
    private func formattedTime(from time: Date) -> String {
        return formatTime(pattern: timeFormat, from: time)
    }

    // Generic function to format date/time
    private func formatTime(pattern: String, from time: Date) -> String {
    ...
    private func eventText(for event: EKEvent) -> String {
        ...
        if !event.isAllDay {
            text += " ("
            text += formattedTime(from: event.startDate)
            text += ")"
        }
        return text
    }
